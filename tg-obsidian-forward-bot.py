...
class Note:
    ...
    Path to structure file should be in config with other options.
    '''
    def select_folder(self):
        try:
            log_basic('Selecting folder for this note...')
            folders_file = config.ollama_folders_file

            log_basic('Loading folders structure from file...')
            ...
            # requesting local ollama instance
            client = ollama.Client(host=config.ollama_url)
            ...
            response = client.generate(
                prompt=structure_select_prompt,
                model=config.ollama_model
            )
            ...
            # Parse JSON response to get selected_path
            result = ''
            try:
                folder_data = json.loads(r)
                result = folder_data.get('selected_path', 'unsorted')
            except json.JSONDecodeError:
                result = 'unsorted'
            log_basic(result)
...
async def process_message(message: types.Message):
    if message.text.startswith('/'): 
        log_basic(f'ignored cmd in chat')    
        return
...
def save_message(note: Note, message: Message) -> None:
    ...
    note_folder_path = os.path.join(config.inbox_path, note_folder)
    if not os.path.exists(note_folder_path): os.makedirs(note_folder_path)
    note_file = os.path.join(note_folder_path, note_title)

    log_basic(f'Received text message from {note_text}')
